...
class WorkspaceCommandLine:
    ...
    def cmd_analyze_total_workspace_exhaustive(self, cmds):
        ...
        if plot:
            ...
            alpha = AlphaShape(sort_cloud(pose_cloud), 1.2)
    ...
    def cmd_analyze_manipulability_within_volume(self, cmds):
        ...
        if self.exhaustive_pose_cloud is not None:
            ...
            if res.lower() == 'y':
                shape = AlphaShape(sort_cloud(self.exhaustive_pose_cloud), 1.2)
        if self.functional_pose_cloud is not None:
            res = input('Would you like to use prior calculated functional pose cloud? Y/N >')
            if res.lower() == 'y':
                shape = AlphaShape(sort_cloud(self.functional_pose_cloud), 1.2)
        ...
        if plot:
            ...
            for r in results:
                ...
                DrawRectangle(
                    ...
                    [grid_resolution]*3, ax, c=col, a=.4)
                #disp('Plotting')
            plt.show()
